...
class MovingObject {
  ...
  getCircleXYatPercent(startPt,endPt,percent) {
    console.log(percent);
        let positionangle = Math.PI*percent;
        var centerx = 490;
        var centery = 350;
        // console.log("angel" + positionangle);
        // console.log(Math.cos(positionangle) * 320);
  ...
  move(timeDelta) {
    if (this.ballname === "fireball"){
      ...
      if(this.percent>1000){ this.percent=1000; this.direction=-1; }
      let xy;
      if(this.percent<25){
             let usepercent=this.percent/24;
             xy=this.getLineXYatPercent({x:820,y:20},{x:810,y:340},usepercent);
         }
      else if(this.percent<75){
             let usepercent=(this.percent-25)/49;
             xy=this.getCircleXYatPercent({x:820,y:360},{x:180,y:360},usepercent);
         }
